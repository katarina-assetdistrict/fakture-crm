      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl border border-red-100 p-5 bg-red-50">
          ...
          <div className="text-xs text-red-400 mt-1">
            {stanja.filter(s => s.dug > 0).length} klijenata duguje
          </div>
        </div>
      </div>

      {/* Stanje po klijentima */}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 text-xs uppercase tracking-wide border-b border-gray-100">
                  ...
                  <th className="px-5 py-3 font-medium text-right">Fakturisano</th>
                  <th className="px-5 py-3 font-medium text-right">Naplaćeno</th>
                  ...
                  <tr key={s.klijent.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      <div className="text-xs text-gray-400">
                        {s.brojFaktura} faktura{s.faktureSaDugom > 0 && `, ${s.faktureSaDugom} neizmirena`}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <StatusBadge dug={s.dug} placeno={s.ukupnoPlaceno} fakturisano={s.ukupnoFakturisano} />
                    </td>
                    <td className="px-5 py-3.5">
                      <Link
                        to={`/klijenti/${s.klijent.id}`}
