...
export default function NovaFaktura() {
  ...
  const { klijenti, firme, addFaktura } = useData();
  ...
  const sacuvaj = async () => {
    ...
    setSaving(true);
    try {
      await addFaktura({ firmaId, klijentId, broj: broj.trim(), datum, datumDospeca, stavke, ukupanIznos: ukupnoIznos, napomena });
      navigate(-1);
    } catch {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {greska && <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg">{greska}</div>}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input value={broj} onChange={e => setBroj(e.target.value)} placeholder="npr. 2024/001" className={inp} />
          </div>
          <div>
        ...
        <div>
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {stavke.map(s => (
                  <tr key={s.id}>
                    <td className="px-2 py-1.5"><input value={s.opis} onChange={e => updateStavka(s.id, 'opis', e.target.value)} placeholder="Opis usluge/robe" className="w-full border border-gray-200 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" /></td>
                    <td className="px-2 py-1.5"><input type="number" min="0" value={s.kolicina} onChange={e => updateStavka(s.id, 'kolicina', Number(e.target.value))} className="w-full border border-gray-200 rounded px-2 py-1 text-sm text-right focus:outline-none focus:ring-1 focus:ring-blue-400" /></td>
                    <td className="px-2 py-1.5"><input type="number" min="0" value={s.cenaPoJedinici} onChange={e => updateStavka(s.id, 'cenaPoJedinici', Number(e.target.value))} className="w-full border border-gray-200 rounded px-2 py-1 text-sm text-right focus:outline-none focus:ring-1 focus:ring-blue-400" /></td>
                    <td className="px-3 py-1.5 text-right font-medium text-gray-900">{formatRSD(s.ukupno)}</td>
                    <td className="px-2 py-1.5">{stavke.length > 1 && <button onClick={() => setStavke(prev => prev.filter(x => x.id !== s.id))} className="text-gray-300 hover:text-red-500"><Trash2 size={13} /></button>}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
          ...
          <button onClick={() => setStavke(prev => [...prev, praznaStavka()])} className="mt-2 flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-sm font-medium">
            <Plus size={14} /> Dodaj stavku
          </button>
        </div>

        <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
          <button onClick={() => navigate(-1)} className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">Otkaži</button>
