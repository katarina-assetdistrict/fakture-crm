...
export default function Fakture() {
  ...
  const fakture = sveFakture
    .filter(f => {
      ...
      const matchFirma = !aktivnaFirma || f.firmaId === aktivnaFirma;
      return matchPretraga && matchGodina && matchKlijent && matchFirma;
    })
    .sort((a, b) => b.datum.localeCompare(a.datum));

  const handleExport = () =>
    exportFaktureExcel(sveFakture, klijenti, firme, sveUplate, aktivnaFirma || undefined, godina);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        <select
          value={lokalnaFirma}
          onChange={e => setLokalnaFirma(e.target.value)}
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        {fakture.length === 0 ? (
          <div className="py-16 text-center text-gray-400 text-sm">
            {pretraga || godina || klijentFilter || aktivnaFirma
              ? 'Nema rezultata za odabrane filtere.'
              : 'Nema faktura. Dodajte prvu fakturu.'}
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 text-xs uppercase tracking-wide border-b border-gray-100">
                  ...
                  <th className="px-4 py-3 font-medium">Dospeće</th>
                  <th className="px-4 py-3 font-medium text-right">Iznos</th>
                  <th className="px-4 py-3 font-medium text-right">Plaćeno</th>
                  <th className="px-4 py-3 font-medium text-right">Dug</th>
                ...
                {fakture.map(f => {
                  ...
                  return (
                    <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          {dug === 0
                            ...
                            : <AlertCircle className={`flex-shrink-0 ${prosloRok ? 'text-red-500' : 'text-amber-500'}`} size={14} />
                          }
                      ...
                      <td className={`px-4 py-3.5 ${prosloRok ? 'text-red-600 font-medium' : 'text-gray-600'}`}>
                        {formatDatum(f.datumDospeca)}
                        {prosloRok && <span className="text-xs ml-1">(isteklo)</span>}
                      </td>
                      <td className="px-4 py-3.5 text-right text-gray-900 font-medium">{formatRSD(f.ukupanIznos)}</td>
